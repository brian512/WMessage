...
public class FileUtil {
    ...
    public static void saveStream2File(ByteArrayOutputStream stream, String savePath) {
        try {
    ...
    public static boolean deleteDir(String path) {
        LogUtil.log("clear dir : " + path);
        if (TextUtils.isEmpty(path)) {
            return false;
        }
        boolean ret = clearDir(path);
        if (ret) {
            File file = new File(path);
            ret = file.delete();
        }
